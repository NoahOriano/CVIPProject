...
def findScaleSpaceExtrema(gaussian_images, dog_images, num_intervals, sigma, image_border_width, contrast_threshold=0.04):
    ...
    for octave_index, dog_images_in_octave in enumerate(dog_images):
        for image_index, (first_image, second_image, third_image) in enumerate(zip(dog_images_in_octave, dog_images_in_octave[1:], dog_images_in_octave[2:])):
            # (i, j) is the center of the 3x3 array used for finding the extrema
...
def isPixelAnExtremum(first_subimage, second_subimage, third_subimage, threshold):
    ...
    absolute_center_value = abs(center_pixel_value)
    if absolute_center_value > threshold:
        return all(absolute_center_value >= abs(first_subimage)) and \
                all(absolute_center_value >= abs(third_subimage)) and \
                all(absolute_center_value >= abs(second_subimage[0, :])) and \
                all(absolute_center_value >= abs(second_subimage[2, :])) and \
                absolute_center_value >= abs(second_subimage[1, 0]) and \
                absolute_center_value >= abs(second_subimage[1, 2])
    return False
...
def generateDescriptors(keypoints, gaussian_images, window_width=4, num_bins=8, scale_multiplier=3, descriptor_max_value=0.2):
    ...
    for keypoint in keypoints:
        ...
        for row_bin, col_bin, magnitude, orientation_bin in zip(row_bin_list, col_bin_list, magnitude_list, orientation_bin_list):
            # Inverse of trilinear interpolation, take the center value of the cube and distribute it among its eight neighbors
        ...
        # Multiply by 512, round, and limit values to between 0 and 255 to convert from float32 to unsigned char
